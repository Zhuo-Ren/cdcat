...
function generateObjListLabelObj(labelDict, curItem)
{
    /* 输入类似于：
    ...
    // <valueObj>
        ...
        if (curItem){
            ...
            // 中间的东西
            {
                ...
                for (let itemIndex = 0; itemIndex<curItem.length; itemIndex++){
                    //
                    let curItemId = curItem[itemIndex];
                    // curItemObj
                    {
                        // 如果是node
                        if(curItemId.slice(0,2) == "n:"){
                            let curItem = getNodeById(curItemId);
                            let curItemObj = $("<button>" + curItemId["text"] + "</button>");
